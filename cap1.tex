...
Dominar LoL é muito desafiador e requer um investimento substancial de tempo \cite{drachen2014skill}, principalmente para equipes inexperientes que não sabem inicialmente como elaborar ou melhorar sua estratégia. Nesse sentido, uma alternativa que poderia ajudar essas equipes \fj{\'e}{seria} fornecer informações que levem a melhores decisões estratégicas no jogo. Um exemplo dessas informações poderia ser métricas de desempenho baseadas no comportamento de equipes bem-sucedidas. Com isso, as equipes poderiam monitorar seus desempenhos ao longo das partidas e verificar se estão no caminho correto ou não. \fj{}{Por exemplo, vamos supor que uma equipe n\~ao tenha focado em melhorar uma determinada m\'etrica de desempenho em suas partidas por algum motivo, como cura efetuada por minuto, no entanto, ao analisar o desempenho de equipes de sucesso, a equipe percebe que a pontua\c{c}\~ao da m\'etrica nas equipes de sucesso est\'a em um n\'ivel alto e que, portanto, pode ter alguma relev\^ancia para melhorar o seu desempenho nas partidas.}
...
\fj{Para preparar o conjunto de dados de desempenho de equipes, foram realizadas as seguintes tarefas: coleta de dados}{Esta se\c{c}\~ao descreve os dados coletados e apresenta uma s\'erie de tarefas executadas para preparar o conjunto de dados de desempenho de equipes: } extração de características; limpeza de dados; remoção de características com baixa variância; \fj{detec\c{c}\~ao}{remo\c{c}\~ao} de \textit{outliers}; transformação de dados; normalização de dados; análise de redundância de características.
...
Em seguida, extraímos características das partidas coletadas para modelar nosso conjunto de dados de desempenho de equipes \fj{}{$X$}. Seja $M$ um conjunto de histórico de partidas onde $|M|=110000$, $T$ um conjunto de equipes onde $|T|=|M| * 2=220000$, $P=\{p_1, \ldots, p_{|P|}\}$ um conjunto \fj{dimensional de}{de desempenho onde cada desempenho $p \in P$ desse conjunto é um vetor de $d=38$} estatísticas básicas de desempenho de um jogador em um partida\fj{}{, tais como \textit{kills}, \textit{mininonsKilled} e \textit{assists}} da Tabela ~\ref{tab:features-desc}. Uma partida consiste de duas equipes distintas $m=\{t_a,t_b\}$ e uma equipe é composta pelo desempenho de 5 jogadores distintos $t=\{p_k |  p_k \in P^t; P^t \supset P; 1 \geq k \leq 5\}$. Selecionamos as estatísticas de desempenho dos jogadores nas partidas e somamos cada estatística $P_j \in P$ por equipe para formar cada característica ou estatística $X_j \in X$ do nosso conjunto de desempenho de equipes \fj{}{$X_{220000, 38}$}, de modo que $X = \{ x_{i} = \sum_{k=1}^{5} p_{k}^{i} | t_{i} \in T; p_{k}^{i} \in P \}$.

Finalmente, para garantir a atomicidade no conjunto de dados $X$, decompomos algumas características, \fj{por exemplo}{ou seja}, subtraimos \textit{physicalDamageDealt} de \textit{physicalDamageDealtToChampions} para computar \textit{physicalDamageDealtToMonsters} \fj{}{(dano físico total causado a monstros)} e subtraímos \textit{magicDamageDealt} de \textit{magicDamageDealtToChampions} para computar \textit{magicDamageDealtToMonsters} \fj{}{(dano m\'agico total causado a monstros)}. Para evitar redundância nos dados, removemos também características compostas, por exemplo, características que são somatórios de outras:
...
Às vezes, os valores de algumas características têm variância zero ou próxima de zero, o que implica que essas características \fj{s\~ao tamb\'em n\~ao}{n\~ao s\~ao} informativas e podem causar ruído ao executar uma determinada tarefa de mineração de dados. Portanto, realizamos uma estatística descritiva no conjunto de dados para identificar e remover características de variância zero ou próxima de zero. Após a análise, o conjunto de dados foi reduzido. As seguintes características foram removidas: \textit{doubleKills}, \textit{inhibitorKills}, \textit {largestMultiKill}, \textit{pentaKills}, \textit {quadraKills}, \textit {sightWardsBoughtInGame} e \textit{tripleKills}.

\section{\fj{Detec\c{c}\~ao}{Remo\c{c}\~ao} de \textit{Outliers}}
\fj{}{A Figura ~\ref{fig:boxplots} ilustra os gráficos dos \textit{boxplots} das caracter\'isticas do conjunto de desempenho de equipes $X$. Como podemos observar, existem muitos valores discrepantes (outliers)}. \fj{Utilizamos um Intervalo Interquartil de $fator = 1,5$ para encontrar os \textit{outliers} de cada caracter\'istica no conjunto de dados.}{} \fj{Equipes}{Então, equipes} com algum\fj{a}{\textit{outlier} em suas} \fj{caracter\'istica}{caracter\'isticas} \fj{an\^omala}{} foram removidas dos dados \fj{}{com base em um Intervalo Interquartil de $fator = 1,5$ para que o comportamento dos dados não fosse afetado de forma brusca}. 

Em seguida, realizamos uma \fj{an\'alise para remover caracter\'isticas com baixa vari\^ancia novamente}{estat\'istica descritiva nos dados novamente e percebemos que algumas caracter\'isticas estavam com vari\^ancia pr\'oxima de zero}. Após \fj{a}{essa} análise, as seguintes características \fj{}{com vari\^ancia pr\'oxima de zero} foram removidas: \textit{totalUnitsHelead} e \textit{visionWardsBoughtInGame}.
...
\fj{Para identificar e remover caracter\'istica redundantes no conjunto de dados, realizamos uma an\'alise de correla\'c{c}\~ao.}{De modo a evitar ru\'idos e apriomorar o tempo de agrupamento dos dados, bem como facilitar a análise dos dados pelos usu\'arios finais, removemos as caracter\'istica redundantes do conjunto de dados por meio de uma an\'alise de correla\c{c}\~ao.} Com base no teste de correlação de Spearman \cite{xiao2015using}, consideramos que um par de características de alta ou altíssima correlação são redundantes. A Figura ~\ref{fig:correlations} ilustra o gráfico de correlação entre as $d=24$ características \fj{}{restantes} do conjunto de dados $X$ \fj{}{reduzido e transformado at\'e est\'a etapa}. De acordo com o gráfico, podemos observar que existem várias características redundantes.

\begin{figure*}
  \centering
  \includegraphics[width=1.0\textwidth]{correlations}%
  \caption{Gráfico de correlação para as características de desempenho das equipes. \fj{}{Cada c\'elula ou n\'umero no gr\'afico representa a correla\c{c}\~ao de um par de caracter\'isticas do conjunto de dados $X$. A escala de cores indica a intensidade das correla\c{c}\~oes, quanto mais forte for uma cor, mais forte é a correla\c{c}\~ao das caracter\'isticas. As cores em tom azul indicam correla\c{c}\~oes positivas. As cores em tom vermelho indicam correla\c{c}\~oes negativas. }}
...
Neste capítulo, caracterizamos \fj{e definimos r\'otulos para}{} os grupos de conjunto de desempenho de equipes de modo a colocar em perspectiva suas principais características ou \fj{estat\'isticas}{m\'etricas} de desempenho \fj{}{e, assim, definir perfis de comportamento}. A fim de compreender como os grupos \fj{}{, ou seja, os perfis,} encontrados se diferem entre si, analisamos:

\begin{enumerate}[label=(\roman*)]
 \item A quantidade de equipes e a \fj{taxa de equipes vencedoras/perdedoras}{propor\c{c}\~ao de equipes vencedoras (taxa de vit\'oria) e a propor\c{c}\~ao de equipes perdedoras (taxa de derrota)};
...
A Figura ~\ref{fig:win-plot} ilustra como os \fj{grupos}{perfis} diferem entre si em termos de quantidade de equipes e taxa de \fj{equipes vencedoras/derrotadas}{vit\'oria/derrota}.

A Figura ~\ref{fig:relevance} ilustra o mapa de calor da análise de relevância das características com base no cálculo do ganho de informação aplicado ao conjunto de métricas desempenho das equipes sem normalização (realizado na seção 4.6) $M$ para indicar como eles influenciam cada \fj{grupo}{perfil}.

Seja $A$ a matriz de centróides dos \fj{grupos}{perfis} das métricas de desempenho das equipes $M$, cada linha $a \in A$ indica os centróides das características de um \fj{grupo}{perfil}, e cada coluna $A_j \in A$ indica os centróides de uma determinada característica através dos \fj{grupos}{perfis}. A Tabela ~\ref{tab:centers} apresenta o conjunto transposto dos centróides $t(A)$ e denota como eles se diferem em termos de desempenho.
...
\caption{Mapa de calor das relevâncias das métricas desempenho das equipes com base no ganho de informação. \fj{}{Quanto mais forte for a cor de uma caracter\'istica, mais intensa \'e a relenvância dessa caracter\'istica.}}
\label{fig:relevance}
\end{figure*}

\begin{table*}
  \tiny
  \caption{Centroides das características (métricas) para cada \fj{cluster}{perfil}. O mínimo e o máximo são indicados por linha.}
  \label{tab:centers}
...
O nível de desempenho moderado consiste dos \fj{Grupo}{Perfil} 2 e \fj{Grupo}{Perfil} 5. Esse nível é distinguido por uma taxa de \fj{equipes vencedoras}{vit\'oria} moderada $w_r = 55 \%$ e uma taxa de \fj{equipes perdedoras}{derrota} baixa $l_r = 45 \%$ (Figura ~\ref{fig:win-plot}). Existem 14 características relevantes para o \fj{Grupo}{Perfil} 2 e 15 características relevantes para o \fj{Grupo}{Perfil} 5 (Figura ~\ref{fig:relevance}). Os mais relevantes ordenados por importância são: \textit{deaths}, \textit{neutralMinionsKilledEnemyJungle}, \textit{killingSprees}, \textit{magicDamageTaken}. A Tabela ~\ref{tab:clusters-moderate} resume as semelhanças e diferenças entre as pontuações das métricas de desempenho para os \fj{grupos}{perfis} desse nível (Figura ~\ref{fig:radars}). A pontuação que mais se diferencia entre os \fj{grupos}{perfis} é: \textit{magicDamageDealtToMonsters} (moderado para o \fj{Grupo}{Perfil} 2 e muito baixo para o \fj{Grupo}{Perfil} 5).

\begin{table}
  \scriptsize
  \caption{Nível das pontuações de desempenho para o \fj{Grupo}{Perfil} 2 e \fj{Grupo}{Perfil} 5.}
...
Nossos resultados demonstram que as equipes das partidas coletadas compartilham diversas semelhanças e diferenças. Identificamos 7 \fj{grupos}{perfis} \fj{distintos de desempenho}{} de equipes que colocam em perspectiva tais semelhanças e diferenças de acordo com um conjunto de estatísticas sumarizadas e transformadas de desempenho de jogadores, tais como, quantidade de mortes, total de dano físico causado, total de cura efetuada, dentre outras. Calculamos, também, para cada \fj{grupo}{perfil}, a taxa de vitória e a taxa de derrota conforme a proporção de equipes vencedoras e derrotadas. Assim, categorizamos esses \fj{grupos}{perfis} em quatro níveis de taxa de vit\'oria da seguinte forma: muito baixo, moderado, alto e muito alto. Em relação à proporção da quantidade de equipes nos \fj{grupos}{perfis}, 28\% das equipes estão no Nível Muito Baixo, 36\% no Nível Moderado, 11\% no Nível Alto e 25\% no Nível Muito Alto. Quanto à relevância das características, as que parecem ter mais influência no comportamento dos \fj{grupos}{perfis} são \textit{deaths}, \textit{killSprees} e \textit{neutralMinionsKilledEnemyJungle}.

\fj{}{Na categoria de N\'ivel Muito Alto de taxa de vit\'oria , encontramos equipes que normalmente apresentam níveis altos de \textit{killSprees}, \textit{minionsKilled}, \textit{neutralMinionsKilled}, \textit{totalHeal} e \textit{wardsPlaced}. Com base nos dados, podemos conceber que essas equipes usam uma estratégia baseada em maximizar a quantidade de minions abatidos por minuto (\textit{minionsKilled}), obtendo vantagem sobre o ouro - que se converte em itens - e pontos de experiência e, assim, aumentanado a probabilidade de \textit{kills} ao combater heróis inimigos. A categoria de N\'ivel Muito Baixo de taxa de vitória pode ser interpretado exatamente o oposto do N\'ivel Muito Alto. As equipes encontradas neste n\'ivel apresentam um nível muito baixo de \textit{minionsKilled}, sendo incapaz de obter itens e tornar-se vulnerável ao ataque dos oponentes. O N\'ivel Moderado apresenta baixos níveis de \textit{deaths}, \textit{magicDamageTaken} e \textit{trueDamageTaken}; isso pode indicar que as equipes desse n\'ivel se concentram muito em estrat\'egias de defesa. O N\'ivel Alto apresenta um n\'ivel baixo  de \textit{deaths}, mas um nível muito alto de \textit{magicDamageDeltToMonters} ou \textit{magicDamageDeltToChampions}; isso pode indicar que a principal estrat\'egia das equipes desse n\'ivel é o ataque surpresa, j\'a que eles n\~ao se concentram tanto em defesa mas possuem poucas mortes.}
